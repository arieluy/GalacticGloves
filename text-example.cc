...
#include "rpi-rgb-led-matrix-master/include/graphics.h"
...
int displayText(int argc, char *argv[], Canvas *canvas, const char* text) {
  ...
  const char *bdf_font_file = "rpi-rgb-led-matrix-master/fonts/7x13.bdf";
  int x_orig = 0;
  int y_orig = 0;
  int brightness = 100;
  ...
  canvas->Clear();
  rgb_matrix::DrawText(canvas, font, x_orig, y_orig + font.baseline(),
                         color, outline_font ? NULL : &bg_color, text,
                         letter_spacing);
